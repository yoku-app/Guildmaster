...
interface OrganisationInviteRepository: JpaRepository<OrganisationInvite, UUID> {
    ...
    @Query("SELECT i FROM OrganisationInvite i WHERE i.userId = :userId")
    ...
    @Query("SELECT i FROM OrganisationInvite i WHERE i.userId = :userId AND i.inviteStatus = :inviteStatus")
